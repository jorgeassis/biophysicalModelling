...
ocean.shape.file <- "_ Atlantic 0.04/ocean.tif"
coast.line.file <- "_ Atlantic 0.04/coast_line.tif"
unwated.regions.file <- "Shapefiles/regionsexcluded.shp" # NULL
new.extent <- NULL # extent( 111.5 , 145 , 18 , 48.5 ) # 

## ------------------------

number.cores <- 5                                 # May crash with all cores assigned
...
for ( simulation.step in unique(available.raw.data$simulation) ) {
                ...
                rm(all.connectivity.pairs.to.sql) ; rm(velocity.field) ; rm(positions.fate) ; rm(particles.reference) ; rm(particles.location.temp) ; rm(particles.location.temp.x) ; rm(particles.location.temp.y) ; rm(particles.location.x) ; rm(particles.location.y) ; rm(raw.data.u) ; rm(raw.data.v) ; rm(speed.u.rk) ; rm(speed.v.rk) ; rm(raw.data.u.t) ; rm(raw.data.v.t)
