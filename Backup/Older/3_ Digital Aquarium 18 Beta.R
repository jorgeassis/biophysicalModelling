...
process.directory <- "/Volumes/Laminaria/Dropbox/Manuscripts/The phylogeography of Laminaria ochroleuca and past climate changes/Results/One Aquarium V2.0"

raw.data.directory <- "Data"
results.directory <- "Results"
results.files <- "Laminaria"
...
ocean.shape.file <- "ocean.tif"
coast.line.file <- "coast_line.tif"
unwated.regions.file <- "Shapefiles/regionsexcluded.shp" # NULL

new.extent <-  extent( -35.0 , 2.5 , 26.5 , 54.0 ) # NULL

## ------------------------

number.cores <- 12                                # May crash with all cores assigned
...
parcticles.to.sql.xy <- "Shapefiles/points_video.shp" # matrix( c(  -2.20 , 36.73 , -5.42 , 36.17 , -15.92 , 23.76 ) , ncol=2 , byrow=TRUE)
parcticles.to.sql.years <- 2012 # 9999
...
# head(available.raw.data[available.raw.data$year==2012,])

for ( simulation.step in unique(available.raw.data$simulation) ) {
                ...
                for( t.step in 1:n.simulation.steps ) {
                        ...
                        if( moving.particles.condition ) {
                                          ...
                                          if( kill.by.raft ) {
                                                    ...
                                                    if( particles.on.land.condition ) {    
                                                              ...
                                                              if( TRUE %in% non.rafters.t ) {    
                                                                ...
                                                                moving.particles.xy.t[id %in% non.rafters.id[non.rafters.t], c("x","y") := list( initial.coords[cell %in% non.rafters.cell[non.rafters.t], x ] , initial.coords[cell %in% non.rafters.cell[non.rafters.t], y ] ) ]
                                                              ...
                                                              # For non-Rafters Old Particles
                                                              # Old version: always try to get to new cell
                                                              #non.rafters.id <- who.at.land.id[displacement == 0]
                                                              #non.rafters.t <- who.at.land.tstart[displacement == 0] < t.step
                                                              #if( TRUE %in% non.rafters.t ) {    
                                  ...
                                  if( parcticles.to.sql.years == simulation.year & TRUE %in% ( particles.to.sql.cells %in% inject.and.remove.cells ) ) {
                                            ...
                                            sql <- dbConnect(RSQLite::SQLite(), paste0(sql.directory,"/",results.files,".particles.video.sql"))
                ...
                rm(all.connectivity.pairs.to.sql) ; rm(particles.reference) ; rm(particles.location.temp) ; rm(particles.location.temp.x) ; rm(particles.location.temp.y) ; rm(particles.location.x) ; rm(particles.location.y) ; rm(raw.data.u) ; rm(raw.data.v) ; rm(speed.u.rk) ; rm(speed.v.rk) ; rm(raw.data.u.t) ; rm(raw.data.v.t)
