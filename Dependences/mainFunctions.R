...
#  "digest", "spatstat", "dismo",  "compiler", "nngeo", "randomcoloR", "factoextra", "geosphere", "av", "countrycode", "spatialEco", "geosphere", "spdep", "rworldxtra", "cleangeo",
# unused "h3-r", "h3r",

packages.to.use <- c("devtools",
                     "compiler",
                     "FNN",
                     "readxl",
                     "rworldmap",
...
for(package in packages.to.use) {
  ...
  if( ! package %in% rownames(installed.packages()) ) { stop("Review :: 001") }
  
  # install.packages(paste0("Dependences/packages/","/MSBVAR.tar.gz"), repos=NULL, type="source")
  ...
  if( ! package %in% rownames(installed.packages()) ) { 
    ...
    requirements <- requirements[grepl(package,requirements)]
    version <- NA
...
exportPackages <- function() {
  ...
  makeRepo(packages.to.use, path = "Dependences/Packages/", type = "source", Rversion = paste0(R.Version()$major,".",R.Version()$minor),quiet=TRUE)
...
download.my.ocean.v2 <- function(outputdir, outputfilename,gateway,fromdate, todate,variables,dataset,servicedatabase,minx,maxx,miny,maxy,min.depth,max.depth) {
  ...
  # --product-id global-reanalysis-phy-001-030-monthly --longitude-min -180 --longitude-max 179.9166717529297 
  # --latitude-min -80 --latitude-max 90 --date-min "2016-11-16" --date-max "2016-12-16 12:00:00" --depth-min 0.493 --depth-max 5727.918000000001 
  # --variable thetao --out-dir <OUTPUT_DIR> --out-name <OUTPUT_FILENAME> 
  # 
  ...
  system2(python.path, 
          paste(motu.path, 
                ...
                "--pwd", paste0("'",password,"'") , 
                "--motu", gateway,
                "--service-id", servicedatabase,
                "--product-id", dataset,
                "--longitude-min", minx, 
                "--longitude-max", maxx, 
                "--latitude-min", miny, 
                "--latitude-max", maxy,
                "--date-min", paste0("'",fromdate,"'") , 
                "--date-max", paste0("'",todate,"'") ,
                "--depth-min" , min.depth , 
                "--depth-max", max.depth, 
                "--variable", variables,
                "--out-dir", outputdir,
