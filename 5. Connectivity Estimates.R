...
colnames(distance.probability) <- c("Pair.from" , "Pair.to" , "Number.events", "Time.mean", "Time.min", "Time.max", "Time.sd", "Probability", "Year","Distance")
...
project.name <- paste0("ID#",sp)
file.sampling.sites <- paste0(project.folder,"/Data/ID#",sp,"_Coords.txt")
file.differentiation <- paste0(project.folder,"/Data/ID#",sp,"_FST.txt")
...
for(n.days in 1:max.days.sim) {
    ...
    connectivity[,5] <- connectivity[,5] + 1e-99
