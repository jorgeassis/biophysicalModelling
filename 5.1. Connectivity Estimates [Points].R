...
for( c in 1:nrow(combinations) ){ #  
  ...
  higherSelfRecruitment <- selfRecruitment
  higherSelfRecruitment[!is.na(higherSelfRecruitment)] <- 0
  higherSelfRecruitment[which(selfRecruitment >=  as.numeric(quantile(selfRecruitment,0.95,na.rm=TRUE)))] <- 1
  higherSelfRecruitmentResults[ sapply(names(higherSelfRecruitment),function(x) which( rownames(higherSelfRecruitmentResults) == x)) ,c] <- "TRUE"
  ...
  isolatedResults[ sapply(names(isolated.sourceSink),function(x) which( rownames(isolatedResults) == x)),c] <- "TRUE"
  ...
  higherResistance <- resistance
  higherResistance[!is.na(higherResistance)] <- 0
  higherResistance[which(resistance >=  as.numeric(quantile(resistance,0.95,na.rm=TRUE)))] <- 1
  higherResistanceResults[ sapply(names(higherResistance),function(x) which( rownames(higherResistanceResults) == x)) ,c] <- "TRUE"
  write.csv(higherResistanceResults,file="../Results/higheResistance.csv")
